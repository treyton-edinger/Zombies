...
static const char *geom_file = "person.cnv";
...
static void draw_neck() {
	...
	glPushMatrix();
		glTranslatef(0, dy, 0);
		draw_part(neck_radius, neck_height, person_slices, person_stacks);
	glPopMatrix();
}
//
static void draw_head() {
	...
	float dy = body_height + leg_height + neck_height/2 + head_radius;
